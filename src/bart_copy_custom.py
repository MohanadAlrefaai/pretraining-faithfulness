...
from transformers import BartForConditionalGeneration, BartModel, AutoConfig
...
class BartCopyGenerator(BartForConditionalGeneration):
    """
    Bart with the copy mechanism of (See, 2017).
    ...
    def __init__(self, config):
        ...
        self.attn_dropout_layer = nn.Dropout(p=0.3)
        self.pgen_context_dropout_layer = nn.Dropout(p=0.3)
        self.pgen_decoder_output_dropout_layer = nn.Dropout(p=0.3)
        self.pgen_decoder_prev_output_dropout_layer = nn.Dropout(p=0.3)
    ...
    def _compute_output_dist(
        ...
        pgen_context = self.pgen_context_dropout_layer(pgen_context)

        pgen_decoder_output = self.pgen_decoder_output_layer(decoder_outputs)
        pgen_decoder_output = self.pgen_decoder_output_dropout_layer(pgen_decoder_output)

        pgen_decoder_prev_output = self.pgen_decoder_prev_output_layer(
            BartCopyGenerator._shift_right_one_pad(decoder_outputs)
        )
        pgen_decoder_prev_output = self.pgen_decoder_prev_output_dropout_layer(pgen_decoder_prev_output)
        ...
        #p_gen = torch.zeros_like(p_gen) + 0.2

        # Compute P_copy (Eq. 9)
        p_copy = torch.zeros_like(p_vocab) #+ 0.9
        ...
        #print("p_gen", torch.max(p_gen))
        #print("p_copy", torch.max(p_copy))
        ...
        if "encoder_outputs" not in model_kwargs:
            ...
            encoder_kwargs = {
                ...
                if not (
                    ...
                    or argument.startswith("labels") or argument.startswith("use_cache")
                )
            }
            encoder_kwargs["output_attentions"] = True
            model_kwargs["encoder_outputs"]: BaseModelOutput = encoder(
