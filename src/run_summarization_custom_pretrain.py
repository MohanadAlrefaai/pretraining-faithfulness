...
def main():
    ...
    # if (data_args.gpu_id != -1):
    #     import torch
    #     torch.cuda.set_device(data_args.gpu_id)
    ...
    for layer in model.model.decoder.layers:
        trainable = True
        if (num_layer < len(model.model.decoder.layers) - 2):
            trainable = False
            #trainable = True
    ...
    def custom_train_preprocess(examples):
        ...
        if (not data_args.ner_mlm) and (not data_args.ner_gsg):
            return examples
        ...
        for i in range(len(examples[text_column])):
            ...
            list_entities = [a.text for a in spacy_pipeline(preprocessed_exp).ents]
            if data_args.ner_mlm:
                for entity in list_entities:
                    if random.random() <= data_args.ner_mlm_prob:
                        preprocessed_exp = preprocessed_exp.replace(entity, "MASK1")

            #if data_args.ner_gsg:
                # TODO
          #  print("example before", examples[text_column][i])
           # print("example after", preprocessed_exp)
            examples[text_column][i] = preprocessed_exp
    ...
    data_collator = DataCollatorForSeq2SeqWithMultipleReferences(
        tokenizer,
        model=model,
    ...
    qe_logdir = os.path.join(current_dir, "..", training_args.output_dir, "logs")
    ...
    def postprocess_text(preds, labels, inputs):
        ...
        inputs = ["\n".join(nltk.sent_tokenize(input)) for input in inputs]
    ...
    def compute_metrics(eval_preds):
        try:
            ...
            inputs = np.where(inputs >= 0, inputs, tokenizer.pad_token_id)
            
            decoded_labels = tokenizer.batch_decode(labels, skip_special_tokens=True)

            decoded_inputs = tokenizer.batch_decode(inputs, skip_special_tokens=True)

            # save predicts before computing metrics
            ...
            if trainer.is_world_process_zero():
                if training_args.predict_with_generate:
                    ...
                    with open(output_prediction_file, "w", encoding="utf-8") as writer:
                        writer.write("\n".join(predictions))
                    print("saved to generated_predictions.txt")
                    
                    import pandas as pd
                    df = pd.DataFrame(np.transpose([decoded_inputs, decoded_labels, decoded_preds]), columns=["inputs", "labels", "preds"])
                    
                    output_pairs_json_file = os.path.join(training_args.output_dir, "pairs_data.json")
                    output_pairs_csv_file = os.path.join(training_args.output_dir, "pairs_data.csv")
                    try:
                        df.to_json(output_pairs_json_file, orient='records')
                    except:
                        print("cannot write to json")
                    try:
                        df.to_csv(output_pairs_csv_file, encoding='utf-8')
                    except:
            ...
            result["BS_precision"] = np.average(bertscore_results["precision"])
            result["BS_recall"] = np.average(bertscore_results["recall"])
            result["BS_f1"] = np.average(bertscore_results["f1"])

            trainer.log_metrics("predict", result)        
            trainer.save_metrics("predict", result)

            print("computing BS_FACT")
            bertscore_fact_results = bertscore.compute(predictions=decoded_preds, references=decoded_inputs, lang='en')
            result["BS_Fact_precision"] = np.average(bertscore_fact_results["precision"])
            result["BS_Fact_recall"] = np.average(bertscore_fact_results["recall"])
            result["BS_Fact_f1"] = np.average(bertscore_fact_results["f1"])
            trainer.log_metrics("predict", result)  
            trainer.save_metrics("predict", result)

            print("computing custom_loss")
            # result["custom_loss"] = np.mean(np.square(np.divide(2, np.array(bertscore_fact_results["precision"]) + 1)))
            result["custom_loss"] = -1 * np.mean(np.array(bertscore_fact_results["precision"]))

            trainer.log_metrics("predict", result)  
           # print("computing QuestEval")
            #questeval_results = questeval.corpus_questeval(decoded_inputs, decoded_preds)
            #result["quest_eval"] = questeval_results['corpus_score']
            #trainer.log_metrics("predict", result)  
            #trainer.save_metrics("predict", result)

            #print("computing FEQA")
            #feqa_results = feqa_score(decoded_inputs, decoded_preds)
            #result["feqa_score"] = feqa_results
            #trainer.log_metrics("predict", result)  
            #trainer.save_metrics("predict", result)
