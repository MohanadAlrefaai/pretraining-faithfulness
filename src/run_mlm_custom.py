...
def main():
    ...
    if data_args.ner_mlm:
        #mlm_dataset =  load_from_disk("C:\.cache\huggingface\datasets\mlm\wiki_mlm")
        mlm_dataset_train =  load_from_disk(r"G:\.cache\huggingface\datasets\mlm\arxiv")
        mlm_dataset_val =  load_from_disk(r"G:\.cache\huggingface\datasets\mlm\arxiv_val")
        mlm_dataset_test =  load_from_disk(r"G:\.cache\huggingface\datasets\mlm\arxiv_test")
       # raw_datasets = mlm_dataset.train_test_split(test_size=0.0001)
        raw_datasets = DatasetDict({ 
            "train": mlm_dataset_train,
            "validation": mlm_dataset_val,
             "test": mlm_dataset_test })
        #raw_datasets["validation"] = raw_datasets["test"]
    else:    
        mlm_sgs_dataset =  load_from_disk("C:\.cache\huggingface\datasets\mlm\wiki_mlm_sgs")
        raw_datasets["validation"] = raw_datasets["test"]
        #raw_datasets = DatasetDict({ "train": mlm_sgs_dataset })
    ...
    if training_args.do_train:
        ...
        #if data_args.start_index != 0:
        #    start_index =  min(len(train_dataset) - 1, data_args.start_index)
