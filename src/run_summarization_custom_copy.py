...
def main():
    ...
    from bart_copy_custom import BartCopyGenerator
    model = BartCopyGenerator.from_pretrained(
    ...
    for layer in model.model.encoder.layers:
        # trainable = False
        trainable = True
        if (num_layer < len(model.model.encoder.layers) - 1):
            # trainable = False
            trainable = True
    ...
    for layer in model.model.decoder.layers:
        trainable = False
        if (num_layer < len(model.model.decoder.layers) - 1):
            trainable = False
            # trainable = True
        for param in layer.parameters():
            param.requires_grad = trainable
